...
void work_in_progress(const int *tokenIndex, char *cmd, char *const *instructions);
...
int main(int argc, char *argv[]) {
    ...
    *validExecution = true;
    ...
    char **arrOfDirectories = malloc(6 * sizeof(char *));
    for (int i = 0; i < 6; i++) {
        arrOfDirectories[i] = NULL;
    }
        ...
        if (instructions[0] != NULL) {
            ...
            if (strcmp(instructions[0], "cd") == 0) {
                ...
                builtin_pwd();
                       ...
                       (strchr(instructions[0], '*') == NULL)) { // should work didn't test yet
                use_fork_basic(cmd, parameters, instructions, validExecution);
            } else if (strchr(curr_payload(), '*') != NULL) { // payload = curr string before it was turned into tokens
                work_in_progress(tokenIndex, cmd, instructions); // I refactored your code into a function using CLion
            } else {
                use_fork_adv(cmd, parameters, instructions, arrOfDirectories, validExecution); // no path given.
            }
        }
...
void work_in_progress(const int *tokenIndex, char *cmd, char *const *instructions) {/*char **args;
int arg_index=0;
